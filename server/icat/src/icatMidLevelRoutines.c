...
int checkObjIdByTicket(char *dataId, char *accessLevel, 
   ...
   if (strncmp(accessLevel, "modify", 6) == 0) {
      ...
      status = cmlGetStringValuesFromSql(query, cVal, iVal, 9, ticketStr, dataId, dataId, icss);
   }
   else {
      ...
      char query [] =
    		  "select ticket_id, uses_limit, uses_count, ticket_expiry_ts, restrictions"
    		  "  from R_TICKET_MAIN"
    		  "  where ticket_string = ?"
    		  "    and (object_id = ? "
    		  "         or object_id in (select coll_id from R_DATA_MAIN where data_id = ?))";
      status = cmlGetStringValuesFromSql(query, cVal, iVal, 5, ticketStr, dataId, dataId, icss);
   }
